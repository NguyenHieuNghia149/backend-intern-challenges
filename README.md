...
- Trong phương pháp của tôi sử dụng hashing để chuyển mã khuyến mãi thành một index duy nhất, sau đó dùng BitSet để đánh dấu sự tồn tại của index đó trong từng file.
...
2. **Quyết định đánh đổi**:
   - Sử dụng BitSet để lưu trữ các mã khuyến mãi là một cách tối ưu về bộ nhớ so với việc lưu hàng triệu chuỗi string vào trong Set.
   - **Sự đánh đổi**: Giải pháp này được tối ưu hóa cho bài toán cụ thể này, với các ràng buộc cố định ("max 5 chars", "a-z"). Nếu yêu cầu thay đổi (ví dụ độ dài mã 10 ký tự, hoặc bao gồm cả số, ký tự in hoa, ký tự đặc biệt), thì không gian lưu trữ sẽ tăng lên rất nhiều. Khi đó BitSet sẽ quá lớn để lưu trữ và giải pháp này sẽ không còn hiệu quả.
