...
I. PHƯƠNG PHÁP TIẾP CẬN( YOUR APPROACH)
    ...
    Trong phương pháp của tôi sử dụng là dùng hashing để chuyển mã khuyến mãi thành một index đuy nhất, sau đó dùng BitSet đề dánh dấu sự tồn tại của index đó trong từng file
    Sau đó, để kiểm tra một mã khuyến mãi có tồn tại trong cả hai file hay không, chỉ cần kiểm tra sự tồn tại của index đó trong cả hai BitSet.
